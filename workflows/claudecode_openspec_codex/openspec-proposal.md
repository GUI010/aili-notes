...
- When drafting `openspec/changes/<id>/tasks.md`, you MUST follow:
  ...
  - GUI/MIXED constraint (mandatory; see `openspec/project.md` → "CLI/GUI/MIXED validation requirements"):
    - GUI verification must be driven via MCP service `playwright-mcp` and evidence must be archived; do NOT use any browser automation scripts (Python/Playwright test runner).
